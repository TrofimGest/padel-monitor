...
- **Ничего не делать.** Настройте cron (ниже) — сбор идёт ежедневно (и сам шлёт
  мгновенные алерты на новые эталоны и снижение цены), отчёт приходит в Telegram
  раз в неделю с картой кандидатов. Отчёт приходит **всегда**, даже пустой; если
  бот молчит неделю — система сломалась.
- **Прочитать отчёт.** Карточка: `[score/100]`, площадь («от X м²» = можно
  арендовать часть), высота, отопление; `📍` адрес · расстояние · метро; цена;
  чеклист ✓/✗/?; вердикт и почему; риски; `🛠` объём ремонта; `💰` цена против
  рынка + доля аренды в выручке; `🎯` ближайший конкурент; `⚖` сравнение с
  прошлыми; `📞` вопросы для звонка; `📷` фото; `🗺` ссылка на карту; ссылка на
  объявление. Блок `👀 На контроле` — варианты, которые вы наблюдаете.
- **Управлять прямо из Telegram** — напишите боту (id варианта берётся из
  карточки отчёта, «id N»):
  - `отпало 175` — скрыть вариант из будущих отчётов (также `скрыть/нет`);
  - `верни 175` — вернуть скрытый;
  - `наблюдать 175` — в блок «На контроле»; `финалист 175`;
  - `звонил 175: высота 7м, торг` — записать итог звонка;
  - `175: заметка` — заметка (её увидит судья);
  - `/top` — топ-5 доступных сейчас; `/watchlist`; `/why 175`; `/help`.

  Команды забираются на каждом сборе и отдельным cron раз в 15 мин
  (`uv run padel-inbox`). Скрытие детерминированное — вариант исчезает из
  отчётов сразу, не дожидаясь судьи.
- **Влиять на подбор без кода (альтернатива)** — свободный текст в
  `pi/preferences.md`: общие вкусы, «не предлагать ТЦ». Судья читает файл
  каждый отчёт.
- **Список конкурентов** — `pi/competitors.md` (падел-клубы Минска). Дополняйте
  руками; судья учитывает близость конкурентов при оценке района.
- **Поменять критерии поиска** — правьте `config.yaml` (секция `profile`:
  площади, высоты, расстояние; `economics`: допущения юнит-экономики), затем
  `uv run padel-rescore`.
- **Разовые ручные команды:**

```bash
uv run padel-enrich                # обогатить топ detail-страницами
uv run padel-candidates            # JSON: кандидаты + метро/конкуренты/экономика
uv run padel-map --candidates /tmp/c.json --out /tmp/m.html   # HTML-карта
uv run padel-telegram --text "тест" # проверить доставку в Telegram
uv run padel-telegram --document /tmp/m.html  # отправить файл (карту)
...
| Госаукционы аренды **au.nca.by** | ✅ проверено: 92 минских лота (спортзалы, дом культуры — часто ≈2 BYN/м²); отдельная секция `auctions`. **Требует BY-IP** (см. ниже) |
...
### Госаукционы au.nca.by и белорусский IP

au.nca.by (каноническая площадка госаукционов аренды — там дешёвая госаренда,
включая спортзалы) отдаёт данные **только на белорусский IP**. Не с BY-адреса —
403, адаптер аккуратно останавливается (`AdapterStop`), остальные источники
работают. Если разработка идёт из-под VPN (напр. Amnezia WG выводит трафик за
пределы РБ), нужен **split-tunnel** — пустить только трафик к au.nca.by мимо
туннеля, через физический BY-шлюз (весь остальной трафик остаётся в VPN):

```
sudo route -n add -host 93.125.23.82 192.168.99.1   # IP au.nca.by / физшлюз
route -n get 93.125.23.82 | grep interface           # должен быть en0, не utun
sudo route delete 93.125.23.82                        # убрать (маршрут непостоянный)
```

IP и шлюз уточни под свою сеть: `dig +short au.nca.by`, `route -n get default`.
На белорусском VPS split-tunnel не нужен — IP и так BY. Подробно —
`pi/site-specs/auctions.md`. Если BY-IP недоступен, выключи источник:
`sources.nca_auctions.enabled: false` в `config.yaml`.
