...
Use this skill first when the user wants to interact with a real website in a real logged-in browser session.

Strong triggers:

- "open this site", "go to this page", "click", "type", "submit", "send", "search on the site"
- tasks on logged-in products such as Telegram Web, Gmail, Google Calendar, Notion, GitHub, WordPress admin, FASTPANEL, Stripe, admin panels, or customer dashboards
- browser-side tasks where the user expects to watch the tab change live
- tasks on arbitrary customer domains where the product is recognizable from the page, such as `wp-admin` or FASTPANEL

Do **not** start by probing Playwright, generic browser dependencies, or unrelated local runtimes if `browser-harness` is installed and the task is clearly a real-browser task. First check whether the harness can attach to the user's Chrome, then proceed there.
